...
class ASPPResNetSE(nn.Module):
    ...
    def __init__(self, inp: int, out: int, nlin: str = 'elu', attention='cse', num_stages=4, sigm_on_end = False, freezed_stages = 0):
        super().__init__()
        self.num_out = out
        stages_params = [
...
def main_debug():
    ...
    with torch.no_grad():
        ...
        print(x_.ndim)
        print(y_.ndim)
        plt.subplot(1, 2, 2), plt.imshow(y_[0]) #Алё, размерность 3!
        plt.show()
    print('-')


if __name__ == '__main__':
    net = ASPPResNetSE(44, 1, num_stages = 4)
    print(net)
