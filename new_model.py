...
#Когда-то это было надо, сейчас видимо нет
...
#Функция активации возвращается вам по строке названия
...
class ASPPResNetSE_mod(nn.Module):

    def __init__(self, inp: int, out: int, nlin: str = 'elu', attention='cse', num_stages=4, sigm_on_end = False, freezed_stages = 0, batch_norm_at_begin = True):
        super().__init__()
        self.num_out = out
        stages_params = [
        ...
        if batch_norm_at_begin:
            body = [nn.BatchNorm2d(stages_params[0]['inp'])] #Убрать в mod2 !!!
        else:
            body = []
        for stage in range(num_stages):
            ...
            stage_res = MultiBottleneckSEBlock(sp['out'], sp['emb'], dilation=sp['dil_res'],
                                               nlin=nlin, num_blocks=sp['numr'], attention=None)#!
            #замораживает freezed_stages нижних слоёв
            if stage < freezed_stages:
                for param in stage_conv.parameters():
                    param.requires_grad = False
                for param in stage_res.parameters():
                    param.requires_grad = False
            #
